...
.controller('QuestoesCtrl', function($scope, $stateParams, $state, $ionicHistory, $ionicConfig, localService, $q, apiService, $ionicPopup) {
    $ionicConfig.backButton.text("");
    $ionicHistory.clearCache();
    // $ionicHistory.clearHistory();
    ...
    function verify(){
        setTimeout(function() {
            if(k < n){ k = n; verify(); }else{ resolve(foto); }
       }, 500);
    }
    ...
    function validaVisitante() {
        return $q(function(resolve, reject) {
            ...
            var rgxMail = /^[a-z0-9.]+@[a-z0-9]+\.[a-z]+(\.[a-z]+)?$/i;
