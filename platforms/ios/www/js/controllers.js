...
.controller('QuestoesCtrl', function($scope, $stateParams, $state, $ionicHistory, $ionicConfig, localService, $q, apiService, $ionicPopup, $sce) {
    ...
    if(url){
      resolveLocalFileSystemURL(url, function(fileEntry) {
        fileEntry.file(function(file) {
          ...
          window.PKVideoThumbnail.createThumbnail (videoUrl, "ignore", opt)
          .then(function(success){
            console.log("success");
            $scope.cadastro.imagem = success;
            $scope.$apply();
          })
          .catch(function(err){
            console.log("err");
          ...
          $scope.cadastro.video = file.localURL;
  ...
  function adSync(data){
    ...
    var alertPp = $ionicPopup.alert({ title: "Dados adicionados com Sucesso!" });
...
.controller('SuccessCtrl', function($scope, $rootScope, $ionicHistory, $state, localService, $ionicPopup, $ionicConfig, apiService) {
  ...
  apiService.getFileContentAsBase64(data.imgUrl,function(base64){
    data.video = base64;
    $scope.send();
  });
  ...
  $scope.send = function(){
    ...
    apiService.postLocal(data, function (res) {
      $scope.sent = true;
      setTimeout(function() { $scope.done = true; }, 500);
    },function(err){
      console.log(err);
      $scope.sent = true;
      setTimeout(function() { $scope.erro = true; }, 500);
    ...
    if(dados && dados.length > 0){
      ...
      if(sendData.imgUrl){
        apiService.getFileContentAsBase64(sendData.imgUrl,function(base64Image){
          sendData.video = base64Image;
      
          setTimeout(function() {
            console.log("sincronizar");
            console.log(sendData);
            apiService.postLocal(sendData, function (res) {
              console.log(res);
              setTimeout(function() {
                wdt = wdt+x;
                el.style.width = wdt + '%';
...
.service('apiService', function($http){
  ...
  var post = function(data, success, failure){
  ...
  //http://192.168.0.104/Api/Values
  var postLocal = function(data, success, failure){
    return $http.post('http://192.168.0.104/Api/Values', data)
    .then(function (result) {
      success(result);
    }, function (error) {
      failure(error);
